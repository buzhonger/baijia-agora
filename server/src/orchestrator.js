...
export async function runAgentTurn({ session, agent, emit, signal, enableTools = true, inAutoflow = false, toolMode = 'normal' }) {
  ...
  if (teammates.length) {
    systemPrompt += `\n\n【团队】你是「${agent.name}」。在场的其他队友有：${teammates.join('、')}。`
      + `\n【身份边界——最重要，务必严格遵守】`
      + `\n• 你【只是】「${agent.name}」这一个成员，只能以自己的身份说【你自己的】一段话。`
      + `\n• 【绝对禁止】替其他队友代答、模仿他们的口吻、或一次性输出多个成员的发言。即使用户的消息里同时提到了好几个队友的名字，你也只回答属于你自己的那一份，其余交给他们本人。`
      + `\n• 【绝对禁止】在你的回复里编造"某某队友说：……"这样的内容。你看不到、也不能替他们说话。`
      + `\n• 只有当用户【明确要求】你帮某个队友起草/代拟内容时，才可以那样做；否则一律只说你自己的观点。`
      + `\n【@点名规则——务必严格遵守】`
