...
export async function runAutoFlow({ session, agents, order, emit, toolMode = 'normal', enableTools = true, mindThinking = false }) {
  ...
  try {
    ...
    while (progressed && !controller.signal.aborted) {
      ...
      for (const agentId of order) {
        ...
        const produced = await runAgentTurn({ session, agent, emit, signal: controller.signal, inAutoflow: true, toolMode, enableTools, mindThinking });
